...
jsPsych.plugins['survey-text-2'] = (function() {
  var plugin = {};
  ...
  plugin.info = {
    ...
    parameters: {
      ...
      global_value: { //my modification added for the global editable initial value
            type: jsPsych.plugins.parameterType.STRING,
            pretty_name: 'globalvalues',
            default: "",
            description: 'default editable value'
      },
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for (var i = 0; i < trial.questions.length; i++) {
      ...
      if (typeof trial.questions[i].value === 'undefined')
        trial.questions[i].value = trial.global_value;
    ...
    display_element.querySelector('#jspsych-survey-text-next').addEventListener('click', function(event) {
      ...
      for(var index=0; index<matches.length; index++){
        ...
        for(var actual_val=0; actual_val<val.length; actual_val++)  {
          ...
          if (typeof trial.global_answer !== null)
            //validation =  ((trial.global_answer).toString().toLowerCase()).includes(val[actual_val].toString().toLowerCase());
            if((val[actual_val].toString().toLowerCase()).includes(trial.global_answer.toString().toLowerCase())|| (val[actual_val].toString().toLowerCase()).includes('next')){            
                validation = true; //if correct answer or if user says next, then validation is true
            } else {
                validation = false; //else validation is false
            }
          ...
          for (var i = 0; i < mistakes.length; i++) {
            ...
            if (trial.questions[index].list_errors[mistakes[i]] !== "" ) {
              ...
              if (trial.questions[index].language === "english")
                message = 'Sorry, this is incorrect! Please try again.';
